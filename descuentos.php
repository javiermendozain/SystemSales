 ...
 */
    ...
    class public_descuentos_public_productosPage extends DetailPage
    {
        ...
        {
            return array(
                new FilterColumn($this->dataset, 'idproducto', 'idproducto', $this->RenderText('Ref. de Productos')),
        ...
        protected function AddFieldColumns(Grid $grid, $withDetails = true)
        {
            ...
            //
            $column = new TextViewColumn('idproducto', 'idproducto', 'Ref. de Productos', $this->dataset);
    ...
    class public_descuentosPage extends Page
    {
        protected function DoBeforeCreate()
        {
            $this->dataset = new TableDataset(
        ...
        {
            return array(
                new FilterColumn($this->dataset, 'iddescuentos', 'iddescuentos', $this->RenderText('Valor del Descuentos')),
                new FilterColumn($this->dataset, 'descpcion_descuento', 'descpcion_descuento', $this->RenderText('Descripción Descuento'))
            );
        }
        ...
        protected function setupQuickFilter(QuickFilter $quickFilter, FixedKeysArray $columns)
        {
            $quickFilter
                ->addColumn($columns['iddescuentos'])
                ->addColumn($columns['descpcion_descuento']);
        }
        ...
        protected function setupFilterBuilder(FilterBuilder $filterBuilder, FixedKeysArray $columns)
        {
            $main_editor = new TextEdit('iddescuentos_edit');
            ...
            $main_editor = new TextEdit('descpcion_descuento_edit');
            $main_editor->SetMaxLength(10);
            $main_editor->SetPlaceholder($this->RenderText('EXAMPLE: 10%'));
            
            $filterBuilder->addColumn(
                $columns['descpcion_descuento'],
                array(
                    FilterConditionOperator::EQUALS => $main_editor,
                    FilterConditionOperator::DOES_NOT_EQUAL => $main_editor,
                    FilterConditionOperator::IS_GREATER_THAN => $main_editor,
        ...
        protected function AddFieldColumns(Grid $grid, $withDetails = true)
        {
            if (GetCurrentUserGrantForDataSource('public.descuentos.public.productos')->HasViewGrant() && $withDetails)
            {
            //
            // View column for public_descuentos_public_productos detail
            //
            $column = new DetailColumn(array('iddescuentos'), 'public.descuentos.public.productos', 'public_descuentos_public_productos_handler', $this->dataset, 'Productos');
            $column->setMinimalVisibility(ColumnVisibility::PHONE);
            $grid->AddViewColumn($column);
            }
            
            //
            // View column for iddescuentos field
            //
            $column = new NumberViewColumn('iddescuentos', 'iddescuentos', 'Valor del Descuentos', $this->dataset);
            $column->SetOrderable(true);
            $column->setNumberAfterDecimal(2);
            $column->setThousandsSeparator(',');
            $column->setDecimalSeparator('.');
            ...
            // View column for descpcion_descuento field
            //
            $column = new TextViewColumn('descpcion_descuento', 'descpcion_descuento', 'Descripción Descuento', $this->dataset);
            $column->SetOrderable(true);
            $column->setMinimalVisibility(ColumnVisibility::PHONE);
            $column->SetDescription($this->RenderText(''));
            $column->SetFixedWidth(null);
        ...
        protected function AddEditColumns(Grid $grid)
        {
            ...
            $editColumn = new CustomEditColumn('Valor del Descuentos', 'iddescuentos', $editor, $this->dataset);
            $validator = new RequiredValidator(StringUtils::Format($this->GetLocalizerCaptions()->GetMessageString('RequiredValidationMessage'), $this->RenderText($editColumn->GetCaption())));
            $editor->GetValidatorCollection()->AddValidator($validator);
            ...
            $editor->SetPlaceholder($this->RenderText('EXAMPLE: 10%'));
            $editColumn = new CustomEditColumn('Descripción Descuento', 'descpcion_descuento', $editor, $this->dataset);
            $validator = new RequiredValidator(StringUtils::Format($this->GetLocalizerCaptions()->GetMessageString('RequiredValidationMessage'), $this->RenderText($editColumn->GetCaption())));
            $editor->GetValidatorCollection()->AddValidator($validator);
            $this->ApplyCommonColumnEditProperties($editColumn);
            $grid->AddEditColumn($editColumn);
        ...
        protected function doRegisterHandlers() {
            $detailPage = new public_descuentos_public_productosPage('public_descuentos_public_productos', $this, array('iddescuentos'), array('iddescuentos'), $this->GetForeignKeyFields(), $this->CreateMasterDetailRecordGrid(), $this->dataset, GetCurrentUserGrantForDataSource('public.descuentos.public.productos'), 'UTF-8');
            $detailPage->SetRecordPermission(GetCurrentUserRecordPermissionsForDataSource('public.descuentos.public.productos'));
            $detailPage->SetTitle('Productos');
            $detailPage->SetMenuLabel('Productos');
            $detailPage->SetHeader(GetPagesHeader());
            $detailPage->SetFooter(GetPagesFooter());
            $detailPage->SetHttpHandlerName('public_descuentos_public_productos_handler');
            $handler = new PageHTTPHandler('public_descuentos_public_productos_handler', $detailPage);
            GetApplication()->RegisterHTTPHandler($handler);
        }
        ...
        protected function doBeforeInsertRecord($page, &$rowData, &$cancel, &$message, &$messageDisplayTime, $tableName)
        {
            if($rowData['iddescuentos']<0){
            $cancel=TRUE;
            $message='VALOR DEL DESCUENTO* INCORECTO';
    ...
    try
    {
        $Page = new public_descuentosPage("public_descuentos", "descuentos.php", GetCurrentUserGrantForDataSource("public.descuentos"), 'UTF-8');
        $Page->SetTitle('Descuentos');
        $Page->SetMenuLabel('Descuentos');
        $Page->SetHeader(GetPagesHeader());
        $Page->SetFooter(GetPagesFooter());
        $Page->SetRecordPermission(GetCurrentUserRecordPermissionsForDataSource("public.descuentos"));
